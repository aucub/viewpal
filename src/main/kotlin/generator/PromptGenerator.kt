...
class PromptGenerator {
    companion object {
        val systemMessage =
            if (Config.config.openAiConfig.topic?.isNotEmpty() == true) {
                "你是一位${Config.config.openAiConfig.topic}领域的专家，我将向我询问面试职位的面试问题。我希望你只作为候选人回答。除非我要求，否则不要写解释，保持回答简洁。注意问题使用语音转录而来，可能存在同音字错误和其他语句错误"
            } else {
